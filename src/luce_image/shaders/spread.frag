...
// dilation), mode 2 a minimum (an erosion; past the edge counts as empty).
// `horizontal` picks the axis; `width`/`height` bound the fetches.
#version 450
...
void main() {
    ...
    float result = params.mode > 1.5 ? 1.0 : 0.0;
    float weights = 0.0;
