// Warp and Puppet Warp, one output cell of a pyramid level: each output texel
// shows the layer where the mesh's field sends it, as liquify.frag, times how
// much the mesh covers it (image 3, n by n like the field; nothing past the
// mesh, a soft rim at its edge). Image 1 is a window of the
...
// Coverage sample (x, y), held to the field.
float covered(ivec2 p) {
    return texelFetch(coverage, clamp(p, ivec2(0), ivec2(int(params.field.z) - 1)), 0).r;
}

// The coverage at `u`, bilinear between samples.
float coverage_at(vec2 u) {
    vec2 f = u - floor(u);
    ivec2 b = ivec2(floor(u));
    return mix(mix(covered(b), covered(b + ivec2(1, 0)), f.x), mix(covered(b + ivec2(0, 1)), covered(b + ivec2(1, 1)), f.x), f.y);
}
