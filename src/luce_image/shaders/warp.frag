...
// position and `size` the source texture's size. Bilinear, premultiplied
// while mixing so transparent neighbours do not darken edges.
#version 450
layout(location = 0) in vec4 vertex_color;
layout(location = 0) out vec4 fragment_color;
layout(push_constant) uniform Params { float a, b, c, d, e, f; vec2 origin; vec2 size; } params;
